...
describe('Flujo básico', () => {
  ...
  test('Admin crea espacio, usuario busca, reserva y cancela', async () => {
    ...
    const endTime = `${date}T12:00:00.000Z`;
    ...
    // Crear reserva con campos actualizados
    const resv = await request(app).post('/api/reservations')
      .set('Authorization', `Bearer ${userToken}`)
      .send({ spaceId, startTime, endTime }); //  Claves corregidas
    ...
    const cancel = await request(app)
      .delete(`/api/reservations/${resv.body.id}`)
      .set('Authorization', `Bearer ${userToken}`);
